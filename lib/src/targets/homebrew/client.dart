...
class HomebrewUpdateAuthority {
  const HomebrewUpdateAuthority.absent({this.replacement})
      : sha256 = null,
        legacyCask = null;
  ...
  const HomebrewUpdateAuthority._(
    this.sha256,
    this.replacement,
    this.legacyCask,
  );
  ...
  /// The old cask coordinate removed atomically with the first formula.
  ///
  /// It is part of the same compare-and-swap authority: rk migrates only an
  /// absent cask or the exact rk-generated bytes it inspected.
  final HomebrewTapFileAuthority? legacyCask;

  HomebrewUpdateAuthority withLegacyCask({
    required String path,
    required String? sha256,
  }) =>
      HomebrewUpdateAuthority._(
        this.sha256,
        replacement,
        HomebrewTapFileAuthority(path: path, sha256: sha256),
      );
  ...
  String? get recoveryBinding {
    ...
    final legacy = legacyCask;
    return '${sha256 ?? 'absent'}:${Sha256.hex(bytes)}:'
        '${legacy?.path ?? 'no-legacy-path'}:'
        '${legacy?.sha256 ?? 'absent'}';
  }
...
class HomebrewTapFileAuthority {
  const HomebrewTapFileAuthority({required this.path, required this.sha256});

  final String path;
  final String? sha256;

  bool accepts(List<int>? bytes) => sha256 == null
      ? bytes == null
      : bytes != null && Sha256.hex(bytes) == sha256;

  String mismatch(List<int>? bytes) {
    final expected = sha256 == null ? 'no file' : 'sha256 $sha256';
    final actual = bytes == null ? 'no file' : 'sha256 ${Sha256.hex(bytes)}';
    return 'expected $expected; the fresh tap has $actual';
  }
}
...
class HomebrewTarget {
  ...
  Future<Inspection> inspect({
    ...
    final result = await tools.run(
      'gh',
      ['api', 'repos/$tap/contents/$tapPath'],
    ...
    if (!result.ok) {
      if (result.summary.contains('(HTTP 404)')) {
        ...
        return readable.ok
            ? Inspection.absent(
                detail: 'no $kind in the tap yet',
                evidence: {'public $kind': 'absent'},
    ...
    return PublicReconciliation.movingChannel(
      ...
      unrecognizedDetail:
          'the Homebrew $kind is not a recognizable rk-generated $kind',
...
class HomebrewTap {
  ...
  Future<TapOutcome> update({
    ...
    final legacyAuthority = authority.legacyCask;
    File? legacyFile;
    List<int>? legacyBytes;
    try {
      existing = file.existsSync() ? file.readAsBytesSync() : null;
      if (legacyAuthority != null) {
        legacyFile = File('$checkout/${legacyAuthority.path}');
        legacyBytes =
            legacyFile.existsSync() ? legacyFile.readAsBytesSync() : null;
      }
    ...
    if (legacyAuthority != null && !legacyAuthority.accepts(legacyBytes)) {
      return TapOutcome.failed(
        'the legacy tap cask changed after rk inspected it; '
        '${legacyAuthority.mismatch(legacyBytes)}. Re-run so rk can inspect '
        'the migration again',
      );
    }
    ...
    if (existing != null &&
        HomebrewTarget._sameBytes(existing, replacement) &&
        legacyBytes == null) {
    ...
    try {
      ...
      if (legacyFile != null && legacyBytes != null) legacyFile.deleteSync();
    } on FileSystemException catch (error) {
      return TapOutcome.failed(
          'the tap migration could not be written: $error');
    }
    ...
    final added = await tools.run(
      ...
      [
        ...
        '--',
        formulaPath,
        if (legacyAuthority != null) legacyAuthority.path,
      ],
