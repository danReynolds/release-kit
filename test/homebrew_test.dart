...
void main() {
  ...
  test('a quote cannot escape into the Ruby', () {
    ...
    expect(formula, contains(r'desc "A \"quoted\" description"'));
  });
  ...
  group('the tap update', () {
    ...
    test('a first-ever formula is written, staged, committed, and pushed',
      ...
      expect(
        tools.calls.any(
          (c) => c == 'git add --all -- Formula/keybay.rb',
        ),
        isTrue,
    ...
    test('the first formula atomically removes the exact legacy cask',
        () async {
      const legacy = '# Generated by rk.\n  version "0.9.0"\n';
      final (tapUpdate, tools) = tap(
        inTap: {'Casks/tool.rb': legacy},
      );
      final outcome = await tapUpdate.update(
        formulaPath: 'Formula/tool.rb',
        contents: 'formula\n',
        message: 'tool 1.0.0',
        authority: const HomebrewUpdateAuthority.absent().withLegacyCask(
          path: 'Casks/tool.rb',
          sha256: Sha256.hex(utf8.encode(legacy)),
        ),
      );
      ...
      expect(File('${scratch.path}/tap/Casks/tool.rb').existsSync(), isFalse);
      expect(
        tools.calls,
        contains('git add --all -- Formula/tool.rb Casks/tool.rb'),
        reason: 'creation and removal must land in the same tap commit',
      );
    });

    test('a changed legacy cask stops migration before either file changes',
        () async {
      const observed = '# Generated by rk.\n  version "0.9.0"\n';
      const changed = '# Generated by rk.\n  version "0.9.1"\n';
      final (tapUpdate, tools) = tap(
        inTap: {'Casks/tool.rb': changed},
      );
      final outcome = await tapUpdate.update(
        formulaPath: 'Formula/tool.rb',
        contents: 'formula\n',
        ...
        authority: const HomebrewUpdateAuthority.absent().withLegacyCask(
          path: 'Casks/tool.rb',
          sha256: Sha256.hex(utf8.encode(observed)),
        ),
      );

      expect(outcome.ok, isFalse);
      expect(outcome.problem, contains('legacy tap cask changed'));
      expect(File('${scratch.path}/tap/Formula/tool.rb').existsSync(), isFalse);
      expect(
        File('${scratch.path}/tap/Casks/tool.rb').readAsStringSync(),
        changed,
      );
      expect(tools.calls.where((call) => call.startsWith('git add')), isEmpty);
      expect(tools.calls.where((call) => call.startsWith('git push')), isEmpty);
    });
